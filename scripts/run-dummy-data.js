...
function generateGPXTrack(runName, elevation) {
  ...
  const trackPoints = coordinates.map(coord => 
    `    <trkpt lat="${coord[1]}" lon="${coord[0]}">
      <ele>${coord[2].toFixed(2)}</ele>
      <time>${new Date(Date.now() - Math.random() * 7 * 24 * 60 * 60 * 1000).toISOString()}</time>
    </trkpt>`
  ).join('\n');
  
  return `<?xml version="1.0" encoding="UTF-8"?>
<gpx version="1.1" creator="HeliRun Data Generator" xmlns="http://www.topografix.com/GPX/1/1" xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance" xsi:schemaLocation="http://www.topografix.com/GPX/1/1 http://www.topografix.com/GPX/1/1/gpx.xsd">
  <metadata>
    <name>${runName}</name>
    <desc>Generated ski run track for testing</desc>
    <author>
      <name>HeliRun Data Generator</name>
      <link href="https://caltopo.com"/>
    </author>
    <time>${new Date().toISOString()}</time>
  </metadata>
  <trk>
    <name>${runName}</name>
    <desc>Generated ski run track</desc>
    <trkseg>
${trackPoints}
    </trkseg>
  </trk>
...
function generateRunData(areaId, subAreaId, runNumber) {
  ...
  return {
    ...
    status,
    statusComment,
    gpxPath: null, // Will be set after upload
...
async function uploadGPXContent(mapId, featureId, gpxContent) {
  const path = `runs/${mapId}/${featureId}.gpx`;
  
  console.log(`Uploading GPX to path: ${path}`);
  ...
  console.log(`Successfully uploaded GPX: ${data?.path}`);
  ...
  if (verifyError || !verifyData || verifyData.length === 0) {
    console.error(`Verification failed for ${mapId}/${featureId}:`, verifyError);
    return false;
  }
  
  console.log(`Verified GPX file exists: ${verifyData[0].name}`);
  return true;
}
  ...
  try {
    ...
    for (const subArea of createdSubAreas) {
      ...
      for (let i = 0; i < runsForThisSubArea; i++) {
        ...
        if (uploadSuccess) {
          // Update the run with the correct GPX path (public URL)
          const supabaseUrl = process.env.NEXT_PUBLIC_SUPABASE_URL;
          const publicGpxUrl = `${supabaseUrl}/storage/v1/object/public/heli-ski-files/runs/${runData.caltopoMapId}/${runData.caltopoFeatureId}.gpx`;
          
          const { error: updateError } = await supabase
            .from('runs')
            .update({ gpx_path: publicGpxUrl })
            .eq('id', run.id);
          
          if (updateError) {
            console.error(`Failed to update GPX path for run ${run.id}:`, updateError);
          }
